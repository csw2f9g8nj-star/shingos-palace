...
const { createBookingActionToken } = require("./action-tokens");
...
function balanceUrl(origin, booking) {
  const token = createBookingActionToken({
    scope: "balance",
    bookingId: booking.id,
    ownerId: booking.owner_id,
    expiresAt: booking.pickup_date ? `${booking.pickup_date}T23:59:59-04:00` : undefined,
    lifetimeSeconds: 180 * 24 * 60 * 60,
  });
  return `${origin}/balance-payment.html?booking_id=${encodeURIComponent(booking.id)}&token=${encodeURIComponent(token)}`;
}
...
function buildDepositConfirmationEmail({ booking, amountPaid, origin }) {
  ...
  const dueHtml = hasRemainingBalance
    ? `
      <div style="margin:22px 0;">
        ...
        <p style="margin:0 0 16px;">${buildActionButton(balanceUrl(origin, booking), "Pay Remaining Balance")}</p>
      </div>
    `
    : "";
  ...
  const text = [
    ...
    hasRemainingBalance ? `Pay Remaining Balance: ${balanceUrl(origin, booking)}` : "",
    "",
...
function buildZellePaymentReceiptEmail({ booking, paymentType, amountPaid, origin }) {
  ...
  const remainingBalance = booking.remaining_balance || "$0";
  const bookingStatus = paymentType === "balance" ? "Fully paid" : "Deposit received";

  const introHtml = `<p style="margin:0 0 18px;">Hi ${escapeHtml(ownerName)},</p>
    <p style="margin:0 0 22px;">We received and verified your Zelle payment of <strong>${escapeHtml(amountPaid)}</strong>. Thank you.</p>`;

  const bodyHtml = `
    <div style="background:#fcf8f1;border:1px solid #eadfce;border-radius:18px;padding:20px;margin:24px 0;">
      ${buildInfoRows([
        { label: "Customer", value: ownerName },
        { label: petLabel(petData.pets), value: petData.namesDisplay },
        { label: "Service", value: serviceLabel(booking.service) },
        { label: "Booking dates", value: reservationDates },
        { label: "Amount received", value: amountPaid },
  ...
  const text = [
    `Hi ${ownerName},`,
    "",
    `We received and verified your Zelle payment of ${amountPaid}. Thank you.`,
    "",
    `Customer: ${ownerName}`,
    `${petLabel(petData.pets)}: ${petData.namesDisplay}`,
    `Service: ${serviceLabel(booking.service)}`,
    `Booking dates: ${reservationDates}`,
  ...
  return {
    subject: "Shingo's Palace — Zelle payment received",
    html,
    text,
  };
}
...
function buildBalanceReminderEmail({ booking, origin }) {
  ...
  const text = [
    ...
    `Due: ${dueDate}`,
    "",
    `Pay Remaining Balance: ${balanceUrl(origin, booking)}`,
    `My Account: ${accountUrl(origin, booking.owner?.email)}`,
  ].join("\n");
