...
async function getOrCreateOwnerForUser(supabase, user, profile = {}) {
  ...
  const metadataFirstName = String(metadata.first_name || metadata.given_name || "").trim();
  const metadataLastName = String(metadata.last_name || metadata.family_name || "").trim();
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      email,
      first_name: profile.first_name || metadataFirstName || firstName || "",
      last_name: profile.last_name || metadataLastName || lastParts.join(" ") || "",
      phone: profile.phone || "",
...
function publicDog(dog) {
  ...
  const bookings = (dog.bookings || []).map((booking) => {
    ...
    const bookingPets = (booking.booking_pets || [])
      .map((item) => ({
    ...
    const petNames = bookingPets.map((pet) => pet.name).filter(Boolean);
    const petNamesDisplay = petNames.length
      ? petNames.length === 1
        ? petNames[0]
        : petNames.length === 2
          ? `${petNames[0]} & ${petNames[1]}`
          : `${petNames.slice(0, -1).join(", ")} & ${petNames[petNames.length - 1]}`
      : booking.booking_pet_summary || "";
