...
const { getAdminClient, handleApiError, normalizeField, publicApiError, requireCustomerUser, sendJson } = require("../../api-utils/supabase");
const { enforceRateLimit } = require("../../api-utils/request-security");
...
module.exports = async function handler(req, res) {
  ...
  if (!enforceRateLimit(req, res, { key: "account-pets", limit: 30, windowMs: 60 * 60 * 1000 })) return;
  ...
  try {
    ...
    if (dogPayload.name.length > 100 || dogPayload.breed.length > 120) {
      throw publicApiError("Please keep the pet name under 100 characters and breed under 120 characters.", 400, "pet_fields_too_long");
    }

    let query = supabase.from("dogs");
    if (dogId && req.method !== "POST") {
      query = query.update(dogPayload).eq("id", dogId).eq("owner_id", owner.id).is("archived_at", null);
    } else {
