const { buildZellePaymentReceiptEmail, sendResendEmail } = require("../../api-utils/booking-emails");
...
const { trustedOrigin } = require("../../api-utils/request-security");
...
async function sendManualPaymentEmail(booking, paymentType, amountPaid, req) {
  ...
  const email = buildZellePaymentReceiptEmail({ booking, paymentType, amountPaid, origin: trustedOrigin(req) });
  try {
...
module.exports = async function handler(req, res) {
  try {
    ...
    const { data: updated, error: updateError } = await supabase
      ...
      .eq("id", booking.id)
      .eq(paymentType === "balance" ? "balance_payment_status" : "payment_status", paymentType === "balance" ? booking.balance_payment_status : booking.payment_status)
      .select(PAYMENT_SELECT)
      .maybeSingle();
    if (updateError) throw updateError;
    if (!updated) {
      throw publicApiError("This payment was already confirmed or changed. Refresh before trying again.", 409, "zelle_confirmation_conflict");
    }
