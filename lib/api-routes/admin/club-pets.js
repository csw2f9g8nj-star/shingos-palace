...
function assertLength(value, maxLength, message) {
...
async function createOwner(supabase, fields, res) {
  ...
  assertLength(firstName, 100, "First name is too long.");
  assertLength(lastName, 100, "Last name is too long.");
  assertLength(email, 254, "Email is too long.");
  assertLength(phone, 40, "Phone number is too long.");
...
async function createPet(supabase, fields, res) {
  ...
  [
    [name, 100, "Pet name is too long."],
    [normalizeField(fields.breed).trim(), 120, "Breed is too long."],
    [normalizeField(fields.age).trim(), 40, "Age is too long."],
    [normalizeField(fields.weight).trim(), 40, "Weight is too long."],
    [normalizeField(fields.size).trim(), 40, "Size is too long."],
    [normalizeField(fields.sex).trim(), 40, "Sex is too long."],
  ].forEach(([value, maxLength, message]) => assertLength(value, maxLength, message));
