...
const ALLOWED_STATUSES = new Set(["Compatible", "Supervised interaction only", "Keep separated"]);
...
async function handler(req, res) {
  ...
  try {
    ...
    const status = normalizeField(fields.status).trim();
    const notes = normalizeField(fields.notes).trim();
    const payload = {
      dog_one_id: dogOneId,
      dog_two_id: dogTwoId,
      status,
    ...
    if (!ALLOWED_STATUSES.has(status)) {
      sendJson(res, 400, { ok: false, error: "Please choose a valid compatibility status." });
      return;
    }
    if (notes.length > 3000) {
      sendJson(res, 400, { ok: false, error: "Compatibility notes are too long." });
      return;
    }
