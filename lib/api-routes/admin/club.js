...
async function requireExistingDog(supabase, dogId) {
  const { data: dog, error } = await supabase
    .from("dogs")
    .select("id,name,pet_type,archived_at")
    .eq("id", dogId)
    .single();
  ...
  if (dog.archived_at) {
    throw publicApiError("Restore this pet before adding it to the Club.", 400, "club_dog_archived");
  }
...
async function saveMatch(supabase, admin, fields) {
  ...
  if (notes.length > 2000) {
    throw publicApiError("Club Match notes are too long.", 400, "club_match_notes_too_long");
  }
